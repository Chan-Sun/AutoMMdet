...
class SymbolTable(object):
    ...
    def _define(self, f, o_len, pure):
        ...
        setattr(self, name, entry)#setattr(对象, 字符串(对象属性), 属性值)
        self._impls[name] = f
        return f
...
def rec_eval(expr, deepcopy_inputs=False, memo=None,
    ...
    memo - optional dictionary of values to use for particular nodes用于特定节点的可选值字典
    ...
    node_inputs = {}
    node_list = []
    dfs(node, node_list, seqset=node_inputs)#dfs-深度优先搜索 node_inputs 120;node_list 120 

    # TODO: optimize dfs to not recurse past the items in memo
    #       this is especially important for evaluating Lambdas
    #       which cause rec_eval to recurse 优化dfs以避免递归超过memo中的项
    #
    # N.B. that Lambdas may expand the graph during the evaluation
    #      so that this iteration may be an incomplete  lambdas在评估期间可能拓展图，因此迭代可能并不完整
    if memo_gc:#True
        clients = {}
        for aa in node_list:
    ...
    todo = deque([topnode])#对象
    while todo:
        if len(todo) > max_program_len:#3 9 15 15 13 12 14 13 19>100000 
        ...
        if node.name == 'switch':
            ...
            waiting_on = [v for v in node_inputs[node] if v not in memo]#每次循环，长度不同 3 7 
        ...
        if waiting_on:
            ...
            args = _args = [memo[v] for v in node.pos_args]#0.01 0.0001
            kwargs = _kwargs = dict(
            ...
            try:
                rval = scope._impls[node.name](*args, **kwargs)#'array_union'
...
def bincount(x, weights=None, minlength=None):
    if np_versions[0] == 1 and np_versions[1] < 6:
        ...
        if np.asarray(x).size:
            return np.bincount(x, weights, minlength)#输入数组x必须为整数dtype
        else:
