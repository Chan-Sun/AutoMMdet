...
@implicit_stochastic
@scope.define
def GMM1(weights, mus, sigmas, low=None, high=None, q=None, rng=None,# low=None, high=None,为界限
         size=()):
    """Sample from truncated 1-D Gaussian Mixture Model 截断一维高斯混合模型的样本"""
...
def adaptive_parzen_normal(mus, prior_weight, prior_mu, prior_sigma,
    ...
    assert maxsigma > 0
    assert minsigma > 0
    assert np.all(sigma > 0), (sigma.min(), minsigma, maxsigma) #所有不为空
...
def ap_uniform_sampler(obs, prior_weight, low, high, size=(), rng=None):
    prior_mu = 0.5 * (high + low) #high.obj
    prior_sigma = 1.0 * (high - low) #low.obj
...
def build_posterior(specs, prior_idxs, prior_vals, obs_idxs, obs_vals,
                    oloss_idxs, oloss_vals, oloss_gamma, prior_weight):#oloss-observed_loss
    """
    ...
    memo = {}
    # map prior RVs(random-variables (prior_vals))  to observations
    obs_memo = {}
    ...
    for nid in prior_vals:
        ...
        obs_below, obs_above = scope.ap_filter_trials(
            ...
            oloss_idxs, oloss_vals, oloss_gamma)#基于vals,loss,构建两个概率分布 返回的与 o_vals相关
    ...
    for node in nodes:
        if node not in memo:
            ...
            if node in obs_memo:
                ...
                fn = adaptive_parzen_samplers[node.name]#e.g 10种分布 ap_uniform_sampler, ap_normal_sampler, etc
                b_args = [obs_below, prior_weight] + aa#离散的点
                named_args = [[kw, memo[arg]]
                              for (kw, arg) in node.named_args]
                b_post = fn(*b_args, **dict(named_args))#转为连续分布 使用adaptive_parzen_normal生成分布，用GMM1采样
                ...
                # AST (Abstract Syntax Trees)生成树型图.  They create the distribution,
                ...
                #GMM1_lpdf(samples, weights, mus, sigmas)
                below_llik = fn_lpdf(*([b_post] + b_post.pos_args), **b_kwargs)#l(x)的概率密度函数
                above_llik = fn_lpdf(*([b_post] + a_post.pos_args), **a_kwargs)#g(x)的概率密度函数

                # improvement = below_llik - above_llik
                # new_node = scope.broadcast_best(b_post, improvement)
                new_node = scope.broadcast_best(b_post, below_llik, above_llik)#新的节点
            ...
            memo[node] = new_node
    post_specs = memo[specs]#生成后验分布中的点
...
def tpe_transform(domain, prior_weight, gamma):
    s_prior_weight = pyll.Literal(float(prior_weight))#1.0
    ...
    return (s_prior_weight, observed, observed_loss,
            specs, idxs, vals)#返回的都是Apply对象 包括子对象


def suggest(new_ids, domain, trials, seed,
            prior_weight=_default_prior_weight,#1.0
            n_startup_jobs=_default_n_startup_jobs,#20
            n_EI_candidates=_default_n_EI_candidates,#24
            gamma=_default_gamma,#0.25
            linear_forgetting=_default_linear_forgetting,
            ):

    new_id = new_ids[0]#从list中提取元素

    t0 = time.time()
    (s_prior_weight, observed, observed_loss, specs, opt_idxs, opt_vals) \
        = tpe_transform(domain, prior_weight, gamma)#返回的都是Apply对象
    tt = time.time() - t0
    ...
    for doc in trials.trials:
        ...
        tid = doc['misc'].get('from_tid', doc['tid'])#0
        loss = domain.loss(doc['result'], doc['spec'])#-0.546
        if loss is None:
            # -- associate infinite loss to new/running/failed jobs
            loss = float('inf')
        else:
            loss = float(loss)
        best_docs_loss.setdefault(tid, loss)#{0：-0.546，1：-0.76}  会不断更新
        if loss <= best_docs_loss[tid]:
            best_docs_loss[tid] = loss
            best_docs[tid] = doc

    tid_docs = list(best_docs.items())#10个键值对
    ...
    if docs:
        logger.info('TPE using %i/%i trials with best loss %f' % (
            len(docs), len(trials), min(best_docs_loss.values())))#使用2/2 with best loss -0.79667
    else:
        logger.info('TPE using 0 trials')

    if len(docs) < n_startup_jobs:#0 1 2 3 4 
        # N.B. THIS SEEDS THE RNG BASED ON THE new_id | rand
        return rand.suggest(new_ids, domain, trials, seed)#anneal.suggest(new_ids, domain, trials, seed)#每次返回一组新解 自动跳转到base.insert_trial_docs
    ...
    if tids:#tids 非空
        ...
        fake_id_0 = max(max(tids), new_id) + 2 #2+2=4
    else:
        # -- weird - we're running the TPE algo from scratch
        assert n_startup_jobs <= 0
        fake_id_0 = new_id + 2

    fake_ids = list(range(fake_id_0, fake_id_0 + n_EI_candidates))#n_start_up+2开始24个值
    ...
    o_idxs_d, o_vals_d = miscs_to_idxs_vals(
        [d['misc'] for d in docs], keys=list(domain.params.keys()))#超参数观测集
    ...
    idxs, vals = pyll.rec_eval([opt_idxs, opt_vals], memo=memo,
                               print_node_on_error=False)#miscs_update_idxs_vals要用

    # -- retrieve the best of the samples and form the return tuple检索最好的样本并形成返回元组
    # the build_posterior makes all specs the same

    rval_specs = [None]  # -- specs are deprecated
    rval_results = [domain.new_result()]
    rval_miscs = [dict(tid=new_id, cmd=domain.cmd, workdir=domain.workdir)]#最好的样本包括序号、参数取值
