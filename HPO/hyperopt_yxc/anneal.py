...
class AnnealingAlgo(SuggestAlgo):
    ...
    def __init__(self, domain, trials, seed,
        ...
        shrink_coef: float
            ...
            points have been explored. 随着采样点的增多，采样邻域的缩小率也随之提高。
        """
        SuggestAlgo.__init__(self, domain, trials, seed=seed)
        self.avg_best_idx = avg_best_idx#2.0
        self.shrink_coef = shrink_coef#0.1
        doc_by_tid = {}
        for doc in trials.trials:
            # get either this docs own tid or the one that it's from
            tid = doc['tid']#tid-test index
            ...
            if loss is None:
                ...
                loss = float(loss)
            doc_by_tid[tid] = (doc, loss)#变成元组捆绑 因为loss和doc不是同一个对象中的
        self.tid_docs_losses = sorted(doc_by_tid.items())#.items 为键值对
        ...
        self.node_tids, self.node_vals = miscs_to_idxs_vals(
            ...
            keys=list(domain.params.keys()))#从miscs中提取idxs,val  return idxs,val 第一次空，均为:{'learning_rate':[],'max_depth':[],}
        self.best_tids = []

    def shrinking(self, label):
        """Return fraction of original search width 原始伸缩范围

        Parameters
        ----------
        label: string
            the name of a hyperparameter
        作用在原始分布上 按概率接受 100*delta_loss可能要改，可以归一化后再乘常数
        """
        
        T = len(self.node_vals[label])#vals = self.node_vals[label] 获取参数具体取值
        #return old_div(1.0, (1.0 + T * self.shrink_coef))            
        if T==1:
            return old_div(1.0, (1.0 + T * self.shrink_coef))
        else:
            tids = self.node_tids[label]
            losses = [self.tid_losses_dct[tid] for tid in tids]#0.7333 0.7266
            delta_loss=(np.min(losses)+np.median(losses))/2#np.percentile(losses, 75) median 
            if -delta_loss<=(-losses[T-1]):
                return old_div(1.0, (1.0 + T * self.shrink_coef))#*delta_loss#变量范围伸缩  1/(1+t*shrink_coef),shrink_coef默认0.1
            else:
                return old_div(1.0, (1.0 + T * self.shrink_coef))*(losses[T-1]/delta_loss)
        
    def choose_ltv(self, label, size):
        """Returns (loss, tid, val) of best/runner-up trial
        从所有试验中选出 label:learning_rate (str);loss 会记录观测集中所有值
        """
        tids = self.node_tids[label]
        ...
        if size == 1:
            ...
            #    trials that was previously chosen (non-independence非独立性
        ...
        #几何分布 在n次伯努利试验中，试验k次才得到第一次成功的机率。详细地说，是：前k-1次皆失败，第k次成功的概率为p
        #rand:     rng = np.random.RandomState(seed)
        good_idx = self.rng.geometric(old_div(1.0, self.avg_best_idx), size=size) - 1#服从几何分布 size=1，生成的点的个数
        good_idx = np.clip(good_idx, 0, len(tids) - 1).astype('int32')#限定在已有观测集中选点 恒为1个.强制下限为0，上限为len(tids) - 1 ； numpy.clip(a, a_min, a_max, out=None) [0]
   
        picks = np.argsort(losses)[good_idx]#x中的元素从小到大排列，提取其对应的 index(索引)，然后输出到y 因为求最小
        picks_loss = np.asarray(losses)[picks]#[-0.746]
        picks_tids = np.asarray(tids)[picks]#[0]
        picks_vals = np.asarray(vals)[picks]#[0.00996554]
    ...
    def on_node_hyperparameter(self, memo, node, label):
        ...
        This function works by delegating委托 to self.hp_HPTYPE functions to
        ...
        如果观测集非空则基于观测集中取点，否则从先验分布中取点

        """
        n_observations = len(self.node_vals[label])
        if n_observations > 0:
            # -- Pick a previous trial on which to base the new sample
            size = memo[node.arg['size']]#1 一直都为1
            loss, tid, val = self.choose_ltv(label, size=size)#对参数及取值进行了升序 -0.746,0,0.00996
            try:
                handler = getattr(self, 'hp_%s' % node.name)
            except AttributeError:
                raise NotImplementedError('Annealing', node.name)
            return handler(memo, node, label, tid, val)#会调用后面改进的概率分布 return array([334])
        else:
    ...
    def hp_randint(self, memo, node, label, tid, val):
        ...
        if val1.size:
            ...
            counts = np.zeros(upper)#(600,)
            prior = 1.0
        prior = self.shrinking(label)#0.90909
        p = (1 - prior) * counts + prior * (old_div(1.0, upper))#与count个数一致#(600,)
        rval = categorical(p=p, upper=upper, rng=self.rng,
                           size=memo[node.arg['size']])#p是概率，upper是上界
        return rval
...
def suggest(new_ids, domain, trials, seed, *args, **kwargs):
    new_id, = new_ids
    return AnnealingAlgo(domain, trials, seed, *args, **kwargs)(new_id)#new_id 0
