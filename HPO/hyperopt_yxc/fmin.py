...
class FMinIter(object):
    ...
    def __init__(self, algo, domain, trials, rstate, asynchronous=None,
        ...
        self.poll_interval_secs = poll_interval_secs#1
        self.max_queue_len = max_queue_len#1
        self.max_evals = max_evals#3
        self.rstate = rstate
    ...
    def serial_evaluate(self, N=-1):#N=N-1 max_evals
        for trial in self.trials._dynamic_trials:
            if trial['state'] == base.JOB_STATE_NEW:
                ...
                spec = base.spec_from_misc(trial['misc'])# 参数新的取值；trial有10个键值对 misc存超参数的值 spec获取需要计算的超参数值
                ctrl = base.Ctrl(self.trials, current_trial=trial)
                try:
                    result = self.domain.evaluate(spec, ctrl)#对新的点代入函数计算 运行计算的 训练第1,2,3折 loss:-0.546,status:ok
                except Exception as e:
                        ...
                        if 0:
                            for d in self.trials.trials:
                                print('trial %i %s %s' % (d['tid'], d['state'],
                                                          d['result'].get('status')))#d['suggest_time']=trial['refresh_time'] = coarse_utcnow()
                        new_trials = algo(new_ids, self.domain, trials,#new_ids:0 1 2
                                          self.rstate.randint(2 ** 31 - 1))#tpe.suggest 字典形式 未看到loss
...
def fmin(fn, space, algo, max_evals, trials=None, rstate=None,
    ...
    rval = FMinIter(algo, domain, trials, max_evals=max_evals,
                    ...
                    show_progressbar=show_progressbar)
    rval.catch_eval_exceptions = catch_eval_exceptions#进入
    rval.exhaust()#run, 显示进度条
    if return_argmin:
