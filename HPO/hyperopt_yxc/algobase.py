...
class ExprEvaluator(object):
    ...
    def set_in_memo(self, memo, k, v):
        ...
        if self.memo_gc:
            ...
            memo[k] = v
            for ii in k.inputs():#k.inputs() 3个参数
...
class SuggestAlgo(ExprEvaluator):
    ...
    def __call__(self, new_id):
        self.rng.seed(self._seed + new_id)#new_id 0 1  self._seed=2124297904 self._seed=554800608 
        memo = self.eval_nodes(
            memo={
                self.domain.s_new_ids: [new_id],
                self.domain.s_rng: self.rng,
            })
        idxs, vals = memo[self.expr]#{'lr':[0],'md':[0]} {'lr':[1],'md':[1]} vars={'lr':[0.0025],'md':[1]}
        new_result = self.domain.new_result()#{'status':'new'}
        new_misc = dict(
            tid=new_id,
            cmd=self.domain.cmd,
            workdir=self.domain.workdir)#产生新的参数取值
        miscs_update_idxs_vals([new_misc], idxs, vals)#更新idxs,vals
        rval = self.trials.new_trial_docs(
            [new_id], [None], [new_result], [new_misc])#内部有10个键值 miscs,tid,state都在里面
        return rval#字典 键值 包含参数值
