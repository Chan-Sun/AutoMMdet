    ...
    for misc in miscs:
        for node_id in idxs:
            try :
                ...
                t_idxs = misc['idxs'][node_id]#list
                t_vals = misc['vals'][node_id]#list
...
class Trials(object):
    ...
    * **misc**: despite generic name, this is currently where the trial's
    ...
    def update_trial(self,hyper_name,best_idx,new_idx ):
        #if best_idx!=new_idx:
        #a=self._trials#lsit
        self._trials[best_idx]['misc']['vals'][hyper_name]=[new_idx]
            #self._trials.idxs_vals[1][hyper_name][best_idx]=new_idx 
        return self
    ...
    def insert_trial_docs(self, docs):
        ...
        docs = [self.assert_valid_trial(SONify(doc))
                for doc in docs]#list 包含10个键值
        return self._insert_trial_docs(docs)

    def new_trial_ids(self, N):#更新ids并转为list
        aa = len(self._ids)#1
        rval = list(range(aa, aa + N))#转为list [1] range(1,2)
        self._ids.update(rval)#更新ids并转为list
        return rval
...
class Domain(object):
    ...
    def __init__(self, fn, expr,
        ...
        for node in pyll.dfs(self.expr):
            if node.name == 'hyperopt_param': #如果在space中，则node.name == 'hyperopt_param'
                label = node.arg['label'].obj #hp括号里面的名称 decay lr1 momentum1 decay2
                if label in self.params:
                    raise DuplicateLabel(label)
                self.params[label] = node.arg['obj']#Apply
        ...
        before = pyll.dfs(self.expr) #131个
        # -- raises exception if expr contains cycles  expr向量化，变成有向图
        ...
        vals_by_label = vh.vals_by_label() #18个变量都拿出来了 此处每个都可以修改变量范围
        ...
        # -- N.B. operates inplace: recursive 递归
